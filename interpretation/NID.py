...
def GenNet_pairwise_interactions_topn(w_input, w_later, mask, n):
    ...
    num_combinations = int(np.round(math.factorial(n) / (math.factorial(2) * math.factorial((n - 2))) ))
    ...
    interaction_ranking = pd.DataFrame(np.array([SNP_coord.flatten(), np.array(strength).flatten()]).T, columns=["coord","strength"]) # changed from the original to make it work
...
def Get_weight_tsang(model, layer_n, masks):
    ...
    mask_count = len(masks)-1  # changed
    w_out = 0

    for i in range(len(model.layers)-1, -1, -1):
        # print(w_out.shape) if type(w_out) == np.ndarray else print(w_out)
        if layer_n == i:

...
class Encoder(object):
    ...
    def save_npy(self, data, save_path=None, info=None, index=None):
        ...
        self.phen_info_dic[str(self.npy_iter) + '_' + self.study_name + '.npy'] = info._data.names[
                                                                                  info._data.start:info._data.finish]
        self.npy_iter += 1
    ...
    def save_hdf5(self, data, save_path=None, info=None, index=None):
        ...
        print(('Saving data to ... {}'.format(
            os.path.join(save_path, str(self.hdf5_iter) + '_' + self.study_name + '_encoded.h5'))))
        ...
        if not os.path.isfile(os.path.join(self.out, 'encode_individuals', self.study_name + '.h5')):
            ...
            chunk.to_hdf(os.path.join(self.out, 'encode_individuals', self.study_name + '.h5'), key='individuals',
                         format='table', min_itemsize=25, complib='zlib', complevel=9)

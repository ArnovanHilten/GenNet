...
class GenotypePLINK(GenotypeHDF5):
    ...
    def convert_probes(self, chunk_size=100000):
        ...
        while True:
            ...
            chunk.to_hdf(os.path.join(self.out, 'probes', self.h5_name), key='probes', format='table',
                         data_columns=True, append=True,
                         complib='zlib', complevel=9, min_itemsize=45)
            gc.collect()
            i += 1
        pd.DataFrame.from_dict(hash_table).to_csv(
            os.path.join(self.out, 'probes', self.file_name + '_hash_table.csv.gz'), index=False, compression='gzip',
            sep='\t')
...
class GenotypeMINIMAC(object):
    ...
    def MACH2hdf5(self, out, id=False, remove_id=False):
        ...
        subprocess.call(['bash', os.path.join(os.environ['HASEDIR'], 'tools', 'minimac2hdf5.sh'),
                         self.reader.folder.path, out, os.environ['HASEDIR'], self.study_name], shell=False,
        ...
        if id:
            if self.cluster:
                ...
                cmd = "qsub -sync y -t 1-{} {} {}".format(N, os.path.join(os.environ['HASEDIR'], 'tools',
                                                                          'qsub_helper.sh'),
                                                          os.path.join(out, 'id_convert.sh'))
                print(cmd)
                proc = subprocess.Popen(cmd, shell=True, stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()
            else:
                proc = subprocess.Popen(['bash', os.path.join(out, 'id_convert.sh')], shell=False,
                                        stderr=subprocess.STDOUT)
            ...
            chunk = np.vstack(
                ((np.arange(0, N_probes, self.split_size) + 1)[:-1], np.arange(0, N_probes, self.split_size)[1:]))
            ...
            if self.cluster:
                print('Submit to cluster!')
                cmd = "qsub -sync y -t 1-{} {} {}".format(N_jobs, os.path.join(os.environ['HASEDIR'], 'tools',
                                                                               'qsub_helper.sh'),
                                                          os.path.join(out, 'minimac_convert.sh'))
                print(cmd)
                proc = subprocess.Popen(cmd, shell=True, stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()
            else:
...
class GenotypeVCF(object):
    ...
    def VCF2hdf5(self, out):
        ...
        if self.cluster:
            print('Submit to cluster!')
            cmd = "qsub -sync y -t 1-{} {} {}".format(N_jobs,
                                                      os.path.join(os.environ['HASEDIR'], 'tools', 'qsub_helper.sh'),
                                                      os.path.join(out, 'vcf_convert.sh'))
            print(cmd)
            proc = subprocess.Popen(cmd, shell=True, stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()
        else:
